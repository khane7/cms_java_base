...
public class LogInController {
	...
	public @ResponseBody Map<String, Object> doLogin ( @RequestParam Map<String, String> mapParams, Model model, HttpSession session ) {
		...
		try {
			...
			BeanAccount beanAccount =  daoAccount.getUsers(mapParams.get("user_id"), mapParams.get("passwd"));
			if ( /*!beanAccount.equals("user_id")*/ beanAccount == null || beanAccount.getIs_delete().equals("Y") == true ) {
			...
			mapSession.put("ss_user_id", beanAccount.getUser_id());
			mapSession.put("ss_user_name", beanAccount.getUser_name() );
			mapSession.put("ss_user", beanAccount);
			//mapSession.put("ss_level", beanAccount.getLevel());
			session.setAttribute("session_user", beanAccount);
	...
	public @ResponseBody Map<String, Object> doLogin4SSO ( @RequestParam Map<String, String> mapParams, Model model, HttpSession session ) {
		...
		try {
			...
			beanAccount.setUser_id(mapParams.get("user_id"));
			beanAccount.setUser_name(mapParams.get("user_name"));
			beanAccount.setIs_admin(mapParams.get("is_admin"));
			beanAccount.setLevel(Integer.parseInt(mapParams.get("level")));
