...
import com.humanwares.cms.Common.Code;
import com.humanwares.cms.Common.PageHelper;
...
public class AccountController{
	...
	public String getUserList (
		...
		try {
			daoUsers.getUserList(searchKey, searchValue, pageNum, limitCount, ordering, sort);
			searchValue = URLDecoder.decode(searchValue);
			int countUsers = daoUsers.getUserCount( searchKey, searchValue );
			...
			model.addAttribute( "UserList", daoUsers.getUserList(searchKey, searchValue, pageNum, Code.PAGE_COUNT, ordering, sort ) );
	...
	public @ResponseBody Map<String, Object> setPassword ( @RequestParam HashMap<String, Object> map, Model model ) {
		...
		try {
			...
			BeanUsers user = daoUsers.getUsers( user_id, passwd );
			...
			daoUsers.setPassword(map.get("user_id").toString(), map.get("txtNewPWChk").toString());
		...
		try {
			...
			if ( map.get("user_id").toString().indexOf("||") > -1 ) {
				...
				for (String item : user_ids) {
					if ( item != null && item != "" ) {
						daoUsers.doDelete(item);
					}
				}
				
			} else {
				daoUsers.doDelete(map.get("user_id").toString());
			}
			resultMap.put("RESULT", "OK");
			resultMap.put("MSG", "1" );
	...
	public String showUserDetail ( @RequestParam HashMap<String, Object> map, Model model ) {
		...
		try {
			...
			if ( map.get("user_id") != null ) {
				...
				user = daoUsers.getUsers( map.get("user_id").toString() );
			} else {
	...
	public @ResponseBody Map<String, Object> saveUser ( @RequestParam HashMap<String, Object> map ) {
		...
		try {
			...
			if ( map.get("idx").toString().equals("0") == true && daoUsers.getUsers(map.get("user_id").toString()) != null ) {
				resultMap.put("RESULT", "FAIL");
