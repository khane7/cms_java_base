...
<ul class="articlelist clearfix">
	...
	<c:forEach var="item" items="${ boardList }" varStatus="status">
		<li>
			<div class="box">
				<figure><img src="${pageContext.request.contextPath}${ item.thumbnail.web_path }/${ item.thumbnail.realfilename }" alt="${ item.title }"></figure>
				<div class="data">
					...
					<c:if test="${ item.opt2 != null && item.opt2 ne '' }">
						<a href="${item.opt2}" target="_blank">
							...
							<p class="info"><span class="date"><fmt:formatDate value="${ item.regdate }" pattern="yyyy-MM-dd"/></span><span class="reporter">${ item.writer }</span></p>
							<p class="cont">${ item.content_text }</p>
						</a>
					</c:if>
