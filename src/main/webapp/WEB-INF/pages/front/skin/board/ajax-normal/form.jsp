...
<table class="tblA_write">
	...
	<tr>
		...
		<td>
			<div class="form-subject">
				<input type="text" name="writer" id="writer" class="intA" style="width:150px;" value="${ sessionScope.session_user.user_name }" <c:if test="${ beanBBS.is_nologin eq 'N'}">readonly</c:if> >
			</div>
		</td>
	</tr>
	...
	<c:if test="${ sessionScope.session_user.user_id == null && ( beanBBS.is_nologin == 'Y' || beanBBS.is_secret == 'Y' ) }">
		<tr>
			<th scope="row"><label for="writer_pw">Password</label></th>
			<td>
				<div class="form-subject">
					<input type="password" name="writer_pw" id="writer_pw" class="intA" style="width:150px;">
				</div>
			</td>
		</tr>
	</c:if>
