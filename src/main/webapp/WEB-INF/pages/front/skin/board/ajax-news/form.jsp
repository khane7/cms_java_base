...
<table class="tblA_write">
	...
	<tr>
		<th scope="row"><label for="writer">사용자명</label></th>
		<td>
			<div class="form-subject">
	...
	<c:if test="${ beanBBS.is_secret == 'Y'}">
		<tr>
			<th scope="row"><label for="is_secret">비밀글</label></th>
			<td>
	...
	<c:if test="${ beanBBS.is_nologin == 'Y' || beanBBS.is_secret == 'Y'}">
		<tr>
			<th scope="row"><label for="writer_pw">비밀번호</label></th>
			<td>
				<div class="form-subject">
	...
	<tr>
		<th scope="row"><label for="ifr-editor">내용</label></th>
		<td>
	...
	<tr style="<c:if test="${ beanBBS.is_file == 'N' }">display:none;</c:if>">
		<th scope="row"><label for="ifr-uploader">첨부파일</label></th>
		<td>
			<%--<textarea name="cont" id="cont" class="sty1"></textarea>--%>
...
<div class="btn_area text-center">
	<a href="javascript:saveBoard();" title="확인" class="btn btn-default"><span class="button">확인</span></a>
	<a href="javascript:goList();" title="취소" class="btn btn-default"><span class="button">취소</span></a>
</div>
</form>
